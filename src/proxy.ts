...
const CSRF_SKIP_PATHS = ['/api/webhooks', '/api/auth', '/api/health', '/api/admin', '/api/clans', '/api/upload', '/api/user', '/api/me', '/api/comments', '/api/reports', '/api/notifications', '/api/collections', '/api/follow', '/api/creator/mangas', '/api/manga', '/api/trash', '/api/ai', '/api/chapters', '/api/cron', '/api/email/inbound', '/api/csp-report', '/api/socket'];
...
const CSP_SUFFIX = `' 'unsafe-eval' https://js.stripe.com https://www.googletagmanager.com https://va.vercel-scripts.com 'report-sample'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob: https://*.vercel-storage.com https://*.blob.vercel-storage.com https://*.public.blob.vercel-storage.com https://ui-avatars.com https://placehold.co https://*.unsplash.com; connect-src 'self' https://api.stripe.com https://*.supabase.co https://*.ingest.sentry.io https://*.google-analytics.com https://analytics.google.com https://www.google.com https://va.vercel-scripts.com; frame-src https://js.stripe.com https://hooks.stripe.com; frame-ancestors 'none'; object-src 'none'; media-src 'self'; worker-src 'self'; base-uri 'self'; form-action 'self'`;
...
function setCSRFCookie(response: NextResponse) {
  ...
  // Non-httpOnly cookie with same value for JS to read and send as header
  response.cookies.set('__csrf_mw_js', token, {
    httpOnly: false,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'strict',
    maxAge: CSRF_COOKIE_MAX_AGE,
